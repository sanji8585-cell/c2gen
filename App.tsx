...
const cleanDeepScriptNarration = (text: string): string => {
  return text
    ...
    // 12. 범용 디렉티브 괄호 제거 — (한글/영문키: 값) 패턴이면 전부 제거
    // Gemini가 미등록 디렉티브(사운드, 효과음, BGM, SFX 등)를 생성해도 나레이션 오염 방지
    .replace(/\([가-힣A-Za-z_]+(?:\s*[가-힣A-Za-z_]*)?\s*:\s*[^)]+\)/g, '')
    // 12b. 플래그 디렉티브 (값 없이 이름만: 이전씬유지, 같은장소, 시간경과)
    .replace(/\((?:이전씬유지|같은장소|시간경과|KEEP_PREV|SAME_PLACE|TIME_PASS)\)/g, '')
    // 13. 끝에 붙은 ") 패턴
    .replace(/"\)\s*$/gm, '')
    // 14. 줄 시작/끝의 홀따옴표
    .replace(/^\s*"\s*/gm, '')
